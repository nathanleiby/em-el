# USAGE
# python train.py --dataset dataset --model cards_model_output --labelbin mlb.pickle
...
def run_model(train_x, train_y, test_x, test_y, num_output_classes, model_output_name='cards_model_output'):
    ...
    checkpoint_callback = ModelCheckpoint(model_output_name, monitor='val_loss', save_best_only=True)
    ...
    return fitted_model
# 
# def plot_model_summary(plot_filename):
# 
# 
#     # plot the training loss and accuracy
#     plt.style.use("ggplot")
#     plt.figure()
#     plt.plot(np.arange(0, EPOCHS), fitted_model.history["loss"], label="train_loss")
#     plt.plot(np.arange(0, EPOCHS), fitted_model.history["val_loss"], label="val_loss")
#     plt.plot(np.arange(0, EPOCHS, fitted_model.history["acc"], label="train_acc")
#     plt.plot(np.arange(0, EPOCHS), fitted_model.history["val_acc"], label="val_acc")
...
if __name__ == "__main__":
    ...
    image_paths, label_map = load_image_and_label_files()
    data, labels, label_binarizer = preprocess_data(image_paths, label_map)
